...
class ImageText():
    ...
    def writeImage(self, image, text, textFont, coordx, coordy, color, anchor='ma', spacing=0):
        if '\n' in text:
            #print('Entrou no multiline')
            self.multiLine(image).multiline_text((coordx, coordy), text, anchor=anchor, font=textFont, fill=color, align="left", spacing=spacing)
        else:
...
class ErrorWindow(Toplevel):
    def __init__(self):
        ...
        self.message = Title(self.window, '', 0, 0, 5, 20, columnspan=2, sticky=EW)
        ...
        self.window.geometry("{}x{}+{}+{}".format(dj[0], dj[1], int(pj[0]), int(pj[1])))
        # Responsividade
...
class AreaFrame(Frame):
    def __init__(self, master, frameTitle, campo, nameFrames, row, column, bgColor="gray", padx=10, pady=10, bd=3):
        ...
        for i in range(self._numFrames):
            ...
            specificFields = ['Título Único:', 'Gêneros:', 'Origem:', 'Plataforma:', 'Data de Estréia:']
            ...
            elif 'Título Duplo' in self._frameTitle:
                newFrame = {
                    "subField": self._nameFrames[i],
                    "labelTitle": Title(self.areaFrame, self._nameFrames[i], 7, columnLabel, pady=5, columnspan=4, sticky=EW, fgColor="white"),
                    "labelCharacter1": Title(self.areaFrame, 'até:', 8, columnLabel, padx=2, bgColor="gray", fgColor="white", sticky=W),
                    "labelCharacter2": Title(self.areaFrame, 'até:', 8, columnCharac2, padx=2, bgColor="gray", fgColor="white", sticky=W),
                    "labelFont": Title(self.areaFrame, "font size 1:", 9, columnLabel, padx=2, bgColor="gray", fgColor="white", columnspan=2, sticky=W),
                    "labelFont2": Title(self.areaFrame, "font size 2:", 10, columnLabel, padx=2, bgColor="gray", fgColor="white", columnspan=2, sticky=W),
                    "labelFont3": Title(self.areaFrame, "font size 3:", 11, columnLabel, padx=2, bgColor="gray", fgColor="white", columnspan=2, sticky=W),
                    "inputCampo": self._campo,
                    "inputCharacter1": Input(self.areaFrame, "5", 8, columnICharac1),
                    "inputCharacter2": Input(self.areaFrame, "5", 8, columnICharac2),
                    "inputFont": Input(self.areaFrame, "6", 9, columnInput, columnspan=2),
                    "inputFont2": Input(self.areaFrame, "6", 10, columnInput, columnspan=2),
                    "inputFont3": Input(self.areaFrame, "6", 11, columnInput, columnspan=2),
                }
            else:
                newFrame = {
                    "subField": self._nameFrames[i],
        ...
        # Coordenadas
        if 'Título Duplo' in self._frameTitle:
            specificLabel1 = Title(self.areaFrame, "Original:", 1, 0, bgColor="gray", fgColor="white", columnspan=4, sticky=EW)
            self.labelCoordx = Title(self.areaFrame, "coordx:", 2, 0, bgColor="gray", fgColor="white", columnspan=round(frameColumnspan/2), sticky=EW)
            self.labelCoordy = Title(self.areaFrame, "coordy:", 2, round(frameColumnspan/2), bgColor="gray", fgColor="white", columnspan=round(frameColumnspan/2))
            self.inputCoordx = Input(self.areaFrame, "15", 3, 0, columnspan=round(frameColumnspan/2))
            self.inputCoordy = Input(self.areaFrame, "15", 3, round(frameColumnspan/2), columnspan=round(frameColumnspan/2))
            specificLabel2 = Title(self.areaFrame, "Em Inglês:", 4, 0, bgColor="gray", fgColor="white", columnspan=4, sticky=EW)
            self.labelCoordxST = Title(self.areaFrame, "coordx:", 5, 0, bgColor="gray", fgColor="white", columnspan=round(frameColumnspan/2), sticky=EW)
            self.labelCoordyST = Title(self.areaFrame, "coordy:", 5, round(frameColumnspan/2), bgColor="gray", fgColor="white", columnspan=round(frameColumnspan/2))
            self.inputCoordxST = Input(self.areaFrame, "15", 6, 0, columnspan=round(frameColumnspan/2))
            self.inputCoordyST = Input(self.areaFrame, "15", 6, round(frameColumnspan/2), columnspan=round(frameColumnspan/2))
        else:
    ...
    def getCoordx(self):
        try:
            ...
            self.inputCoordx.setText(0)
            return 0
    
    def setCoordx(self, newCoordx):
        self.inputCoordx.setText(newCoordx)
    
    def getCoordxST(self):
        try:
            return int(self.inputCoordxST.getValue())
        except ValueError:
            self.error().valueError()
            self.inputCoordxST.setText(0)
            return 0
    
    def setCoordxST(self, newCoordxST):
        self.inputCoordxST.setText(newCoordxST)
    ...
    def getCoordy(self):
        try:
            ...
            self.error().valueError()
            self.inputCoordy.setText(0)
            return 0
    
    def setCoordy(self, newCoordy):
        self.inputCoordy.setText(newCoordy)
    
    def getCoordyST(self):
        try:
            return int(self.inputCoordyST.getValue())
        except ValueError:
            self.error().valueError()
            self.inputCoordyST.setText(0)
            return 0
    
    def setCoordyST(self, newCoordyST):
        self.inputCoordyST.setText(newCoordyST)
...
class ActionButton(Button):
    def __init__(self, master, text, bgColor, fontColor, action, row, column, width='40', sticky=N, span=1, listInputs=[], paths='', mColor=(255,255,255), sColor=(255,255,255)):
        super().__init__()
        self.__master = master
        self._action = action
    ...
    def setPreview(self):
        ...
        if secondTitle:
            ...
            for item in self.listInputs:
                field = item['field']
                if field == 'title':
                    textFT = item['frame'].getText()
                    continue
                elif field == 'doubleTitle':
                    coordx = item['frame'].getCoordx()
                    coordy = item['frame'].getCoordy()
                    coordxST = item['frame'].getCoordxST()
                    coordyST = item['frame'].getCoordyST()
                    if item['frame'].getText() == '==':
                        textST = ''
                    else:
                        textST = item['frame'].getText()
                else:
                    continue
                logging.info(f'Configurações do Campo {field}')
                logging.info(f'Coordenadas Título Original (x, y): ({coordx}, {coordy})')
                logging.info(f'Coordenadas Título em Inglês (x, y): ({coordxST}, {coordyST})')
                logging.info(f'Texto do Título Original: {textFT}')
                logging.info(f'Texto do Título em Inglês: {textST}')
                logging.info(f'Tamanho de Texto de Título Original: {len(textFT)}')
                logging.info(f'Tamanho de Texto de Título em Inglês: {len(textST)}')
                for frames in item['frame'].getSubFrames():
                    maxFont = int(frames['inputCharacter1'].getValue())
                    maxFont2 = int(frames['inputCharacter2'].getValue())
                    fontHeight1 = int(frames['inputFont'].getValue())
                    fontHeight2 = int(frames['inputFont2'].getValue())
                    fontHeight3 = int(frames['inputFont3'].getValue())

                # Fonte Primeiro Título
                if len(textFT) < maxFont:
                    previewFontHeightFT = round((fontHeight1 / 1080) * 300)
                elif len(textFT) > maxFont and len(textFT) <= maxFont2:
                    previewFontHeightFT = round((fontHeight2 / 1080) * 300)
                elif len(textFT) > maxFont2:
                    previewFontHeightFT = round((fontHeight3 / 1080) * 300)
                # Font Segundo Título
                if len(textST) < maxFont:
                    previewFontHeightST = round((fontHeight1 / 1080) * 300)
                elif len(textST) > maxFont and len(textST) <= maxFont2:
                    previewFontHeightST = round((fontHeight2 / 1080) * 300)
                elif len(textST) > maxFont2:
                    previewFontHeightST = round((fontHeight3 / 1080) * 300)
                
                previewCoordx = (int(coordx) / 1920 * 523) + 5
                previewCoordxST = (int(coordxST) / 1920 * 523) + 5
                previewCoordy = int(coordy) / 1080 * 300
                previewCoordyST = int(coordyST) / 1080 * 300
                
                # Definindo Fonte
                textFontFT = self.draw.fontText(self.pathMainFont, previewFontHeightFT)
                textFontST = self.draw.fontText(self.pathItalicFont, previewFontHeightST, 'italic')
                # Editando a imagem
                self.draw.writeImage(previewImage, textFT, textFontFT, previewCoordx, previewCoordy, color=self.mainColor, anchor='lm', spacing=0)
                self.draw.writeImage(previewImage, textST, textFontST, previewCoordxST, previewCoordyST, color=self.secondaryColor, anchor='lm', spacing=0)

        for item in self.listInputs:
            field = item['field']
            if field == 'title' and not oneTitle or field == 'doubleTitle':
                continue
            coordx = item['frame'].getCoordx()
            coordy = item['frame'].getCoordy()
            ...
            if field == 'title':
                ...
                previewCoordx = (int(coordx) + 28) / 1920 * 523
            elif field == 'platform':
                previewCoordx = (int(coordx) + 7) / 1920 * 523
            else:
                previewCoordx = (int(coordx) / 1920 * 523) + 2
            if 'Animes' in field:
                previewCoordy = (int(coordy) - 2) / 1080 * 300
            elif 'Name' in field or 'origin' in field or 'genders' in field or 'platform' in field:
                previewCoordy = (int(coordy) + 16) / 1080 * 300
            else:
                previewCoordy = int(coordy) / 1080 * 300
            ...
            if 'Animes' in field:
                ...
                # Editando a imagem
                self.draw.writeImage(previewImage, textToAdd, textFont, previewCoordx, previewCoordy, color=self.secondaryColor, anchor='la', spacing=textSpacing if textSpacing else 0)
            else:
                textFont = self.draw.fontText(self.pathMainFont, previewFontHeight)
                # Editando a imagem
                if 'title' in field and oneTitle or 'premiere' in field:
                    textAnchor = 'mm'
                else:
                    textAnchor = 'lm'
                self.draw.writeImage(previewImage, textToAdd, textFont, previewCoordx, previewCoordy, color=self.mainColor, anchor=textAnchor, spacing=textSpacing if textSpacing else 0)
        # Salvando a imagem
    ...
    def setColors(self, newColors=''):
        self.mainColor = newColors[0] if newColors else ''
        self.secondaryColor = newColors[1] if newColors else ''
    ...
    def saveConfigs(self):
        ...
        with open(os.path.join(locationPath, r'src\data\configs.txt'), 'w') as configs:
            for item in self.listInputs:
                ...
                if 'Título Duplo' in field:
                    configs.write(f"Ecoordx: {item['frame'].getCoordxST()}\n")
                    configs.write(f"Ecoordy: {item['frame'].getCoordyST()}\n")
                for frames in item['frame'].getSubFrames():
                    if frames['subField'] == '1 linha':
                        ...
                        if 'Animes' not in field:
                            if 'Data' in field:
                                ...
                                configs.write(f"1inputFont2: {frames['inputFont2'].getValue()}\n")
                                if 'Título Duplo' in field:
                                    configs.write(f"1inputFont3: {frames['inputFont3'].getValue()}\n")
                    elif frames['subField'] == '2 linhas':
                        if 'Animes' in field:
    ...
    def orderInfo(self):
        ...
        for x in range(round(len(infoAnimes)/12)):
            animesItems = infoAnimes[x*12:]
            item = 0
        ...
        for anime in animesList:
            ...
            for key, value in anime.items():
                for item in self.listInputs:
                    if key == item['field']:
                        field = item['field']
                        if field == 'title' and not oneTitle:
                            textFT = value
                            continue
                        elif field == 'doubleTitle' and not oneTitle:
                            coordx = item['frame'].getCoordx()
                            coordy = item['frame'].getCoordy()
                            coordxST = item['frame'].getCoordxST()
                            coordyST = item['frame'].getCoordyST()
                            textST = value
                            text = '' # Prevenir erro com len(text)
                        else:
                            coordx = item['frame'].getCoordx()
                            coordy = item['frame'].getCoordy()
                            text = value
                        for frames in item['frame'].getSubFrames():
                            if frames['subField'] == '1 linha':
                                ...
                                try:
                                    ...
                                    fontHeight2 = 0
                                    fontHeight3 = 500
                                if field == 'doubleTitle' and not oneTitle:
                                    maxFont = int(frames['inputCharacter1'].getValue())
                                    maxFont2 = int(frames['inputCharacter2'].getValue())
                                    fontHeight1 = int(frames['inputFont'].getValue())
                                    fontHeight2 = int(frames['inputFont2'].getValue())
                        ...
                        # Fontes caso Título Duplo
                        # Fonte Primeiro Título
                        if field == 'doubleTitle' and not oneTitle:
                            if len(textFT) < maxFont:
                                finalFontHeightFT = fontHeight1
                            elif len(textFT) > maxFont and len(textFT) <= maxFont2:
                                finalFontHeightFT = fontHeight2
                            elif len(textFT) > maxFont2:
                                finalFontHeightFT = fontHeight3
                            # Font Segundo Título
                            if len(textST) < maxFont:
                                finalFontHeightST = fontHeight1
                            elif len(textST) > maxFont and len(textST) <= maxFont2:
                                finalFontHeightST = fontHeight2
                            elif len(textST) > maxFont2:
                                finalFontHeightST = fontHeight3
                        
                        finalCoordx = int(coordx)
                        finalCoordy = int(coordy)
                        if field == 'doubleTitle' and not oneTitle:
                            finalCoordxST = coordxST
                            finalCoordyST = coordyST

                        # Definindo a fonte
                        if 'Animes' in field:
                            textFont = self.draw.fontText(self.pathItalicFont, finalFontHeight, 'italic')
                            # Editando a imagem
                            self.draw.writeImage(finalImage, textToAdd, textFont, finalCoordx, finalCoordy, color=self.secondaryColor, anchor='la', spacing=textSpacing if textSpacing else 0)
                        elif field == 'doubleTitle' and not oneTitle:
                            textFontFT = self.draw.fontText(self.pathMainFont, finalFontHeightFT)
                            textFontST = self.draw.fontText(self.pathItalicFont, finalFontHeightST, 'italic')
                            self.draw.writeImage(finalImage, textFT, textFontFT, finalCoordx, finalCoordy, color=self.mainColor, anchor='lm', spacing=0)
                            self.draw.writeImage(finalImage, textST, textFontST, finalCoordxST, finalCoordyST, self.secondaryColor, anchor='lm', spacing=0)
                        else:
                            textFont = self.draw.fontText(self.pathMainFont, finalFontHeight)
                            # Editando a imagem
                            self.draw.writeImage(finalImage, textToAdd, textFont, finalCoordx, finalCoordy, color=self.mainColor, anchor='mm' if field == 'title' or field == 'premiere' else 'la', spacing=textSpacing if textSpacing else 0)
            # Salvando a imagem
...
async def verifyInputs():
    while True:
        try:
            # Verificando e atualizando cores
            for item in textColors:
                global mainColor, secondaryColor
                if item['field'] == 'mainColor':
                    mainColor = (int(item['redColorInput'].getValue()), int(item['greenColorInput'].getValue()), int(item['blueColorInput'].getValue()))
                elif item['field'] == 'secondaryColor':
                    secondaryColor = (int(item['redColorInput'].getValue()), int(item['greenColorInput'].getValue()), int(item['blueColorInput'].getValue()))
                demoButton.setColors((mainColor, secondaryColor))
                runButton.setColors((mainColor, secondaryColor))
            for item in inputs:
        ...
        try:
            for item in previewInputs:
                if item['input'].getValue():
                    ...
                    item['size'].setName(f"T: 0")
            # Tamanho nos campos de frame
...
if __name__ == '__main__':
    ...
    previewInputs = [
        {
            "field": "title",
            "title":Title(previewTexts, "1. Título Original:", 0, 1, padx=5, sticky=W),
            "input":Input(previewTexts, "40", 0, 2),
            "size":Title(previewTexts, "", 0, 3),
        },
        {
            "field": "secondTitle",
            "title":Title(previewTexts, "2. Título em Inglês:", 1, 1, padx=5, sticky=W),
            "input":Input(previewTexts, "40", 1, 2),
            "size":Title(previewTexts, "", 1, 3),
        },
        {
            "field": "genders",
            "title":Title(previewTexts, "3. Gêneros:", 2, 1, padx=5, sticky=W),
            "input":Input(previewTexts, "40", 2, 2),
            "size":Title(previewTexts, "", 2, 3),
        },
        {
            "field": "studioName",
            "title":Title(previewTexts, "4. Nome do Estúdio:", 3, 1, padx=5, sticky=W),
            "input":Input(previewTexts, "40", 3, 2),
            "size":Title(previewTexts, "", 3, 3),
        },
        {
            "field": "studioAnimes",
            "title":Title(previewTexts, "5. Animes do Estúdio:", 4, 1, padx=5, sticky=W),
            "input":Input(previewTexts, "40", 4, 2),
            "size":Title(previewTexts, "", 4, 3),
        },
        {
            "field": "directorName",
            "title":Title(previewTexts, "6. Nome do Diretor:", 5, 1, padx=5, sticky=W),
            "input":Input(previewTexts, "40", 5, 2),
            "size":Title(previewTexts, "", 5, 3),
        },
        {
            "field": "directorAnimes",
            "title":Title(previewTexts, "7. Obras do Diretor:", 0, 4, padx=5, sticky=W),
            "input":Input(previewTexts, "40", 0, 5),
            "size":Title(previewTexts, "", 0, 6),
        },
        {
            "field": "composerName",
            "title":Title(previewTexts, "8. Nome do Compositor:", 1, 4, padx=5, sticky=W),
            "input":Input(previewTexts, "40", 1, 5),
            "size":Title(previewTexts, "", 1, 6),
        },
        {
            "field": "composerAnimes",
            "title":Title(previewTexts, "9. Obras do Compositor:", 2, 4, padx=5, sticky=W),
            "input":Input(previewTexts, "40", 2, 5),
            "size":Title(previewTexts, "", 2, 6),
        },
        {
            "field": "originalSource",
            "title":Title(previewTexts, "10. Origem:", 3, 4, padx=5, sticky=W),
            "input":Input(previewTexts, "40", 3, 5),
            "size":Title(previewTexts, "", 3, 6),
        },
        {
            "field": "platform",
            "title":Title(previewTexts, "11. Plataforma:", 4, 4, padx=5, sticky=W),
            "input":Input(previewTexts, "40", 4, 5),
            "size":Title(previewTexts, "", 4, 6),
        },
        {
            "field": "premiere",
            "title":Title(previewTexts, "12. Data de Estréia:", 5, 4, padx=5, sticky=W),
            "input":Input(previewTexts, "40", 5, 5),
            "size":Title(previewTexts, "", 5, 6),
        },
    ]
    # Texto de Teste
    ...
    #---------------------------------------------------------------------------
    # Definindo Frame de Especificações Extras
    specificationFrame = Frame(root, width=dj[0], padx=5, pady=5, bd=3, relief="groove")

    textColors = [
        {
            "field": "mainColor",
            "title":Title(specificationFrame, "Cor Principal:", 1, 1, padx=5, sticky=W),
            "redColorLabel":Title(specificationFrame, 'R:', 1, 2, sticky=W),
            "redColorInput":Input(specificationFrame, "5", 1, 3, '255'),
            "greenColorLabel":Title(specificationFrame, 'G:', 1, 4, sticky=W),
            "greenColorInput":Input(specificationFrame, "5", 1, 5, '255'),
            "blueColorLabel":Title(specificationFrame, 'B:', 1, 6, sticky=W),
            "blueColorInput":Input(specificationFrame, "5", 1, 7, '255'),
        },
        {
            "field": "secondaryColor",
            "title":Title(specificationFrame, "Cor Secundária:", 1, 9, padx=5, sticky=W),
            "redColorLabel":Title(specificationFrame, 'R:', 1, 10, sticky=W),
            "redColorInput":Input(specificationFrame, "5", 1, 11, '245'),
            "greenColorLabel":Title(specificationFrame, 'G:', 1, 12, sticky=W),
            "greenColorInput":Input(specificationFrame, "5", 1, 13, '245'),
            "blueColorLabel":Title(specificationFrame, 'B:', 1, 14, sticky=W),
            "blueColorInput":Input(specificationFrame, "5", 1, 15, '245'),
        },
    ]
    # Colors
    mainColor = (int(textColors[0]['redColorInput'].getValue()), int(textColors[0]['greenColorInput'].getValue()), int(textColors[0]['blueColorInput'].getValue()))
    secondaryColor = (int(textColors[1]['redColorInput'].getValue()), int(textColors[0]['greenColorInput'].getValue()), int(textColors[0]['blueColorInput'].getValue()))
    ...
    previewFrames = [
        {
            "field": "title",
            "frame": AreaFrame(adjustFrames, "Título Único:", previewInputs[0]['input'], ("1 linha", "2 linhas"), 0, 0)
        },
        {
            "field": "doubleTitle",
            "frame": AreaFrame(adjustFrames, "Título Duplo:", previewInputs[1]['input'], ("1 linha",), 1, 0)
        },
        {
            "field": "genders",
            "frame": AreaFrame(adjustFrames, "Gêneros:", previewInputs[2]['input'], ("1 linha", "2 linhas"), 2, 0)
        },
        {
            "field": "studioName",
            "frame": AreaFrame(adjustFrames, "Nome do Estúdio:", previewInputs[3]['input'], ("1 linha",), 3, 0)
        },
        {
            "field": "studioAnimes",
            "frame": AreaFrame(adjustFrames, "Animes do Estúdio:", previewInputs[4]['input'], ("1 linha", "2 linhas", "3 linhas", "4 linhas"), 4, 0)
        },
        {
            "field": "directorName",
            "frame": AreaFrame(adjustFrames, "Nome do Diretor:", previewInputs[5]['input'], ("1 linha",), 5, 0)
        },
        {
            "field": "directorAnimes",
            "frame": AreaFrame(adjustFrames, "Animes do Diretor:", previewInputs[6]['input'], ("1 linha", "2 linhas", "3 linhas", "4 linhas"), 6, 0)
        },
        {
            "field": "composerName",
            "frame": AreaFrame(adjustFrames, "Nome do Compositor:", previewInputs[7]['input'], ("1 linha",), 7, 0)
        },
        {
            "field": "composerAnimes",
            "frame": AreaFrame(adjustFrames, "Animes do Compositor:", previewInputs[8]['input'], ("1 linha", "2 linhas", "3 linhas", "4 linhas"), 8, 0)
        },
        {
            "field": "originalSource",
            "frame": AreaFrame(adjustFrames, "Origem:", previewInputs[9]['input'], ("1 linha",), 9, 0)
        },        
        {
            "field": "platform",
            "frame": AreaFrame(adjustFrames, "Plataforma:", previewInputs[10]['input'], ("1 linha",), 10, 0)
        },        
        {
            "field": "premiere",
            "frame": AreaFrame(adjustFrames, "Data de Estréia:", previewInputs[11]['input'], ("1 linha",), 11, 0)
        },        
    ...
    adjustCanvas.grid(row=4, column=0, sticky=EW)
        ...
        for item in previewFrames:
            ...
            for config in configs:
                if item['frame'].getTitle() == config['campo']:
                    ...
                    if 'Título Duplo' in item['frame'].getTitle():
                        item['frame'].setCoordxST(config['Ecoordx'])
                        item['frame'].setCoordyST(config['Ecoordy'])
                    for frame in item['frame'].getSubFrames():
                        if frame['subField'] == '1 linha':
    ...
    specificationFrame.grid_columnconfigure([0,16], weight=1)
